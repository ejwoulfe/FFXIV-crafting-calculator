...
export default function RecipesList() {
  ...
  function createRecipesList(list: Array<RecipeObject>, controller: AbortController) {
    return list.map((recipe: RecipeObject, index: number) => {
      return <RecipeRow data={{ recipe, controller }} key={"recipe-row-" + index} />;
    });
  }
  ...
  return (
    <div id="list-container">
      <div id="pagination-and-filter">
        {recipes !== undefined && filteredList.length > 0 ? (
          <Pagination data={{ totalRecipes, abortController }} />
        ) : null}

        {recipes !== undefined ? <Filter data={{ abortController, setAbortController }} /> : null}
      </div>
      ...
      <div id="rows-container">
        {loading === false && filteredList.length > 0 ? (
          ...
          <span className="no-results">0 results found.</span>
        )}
      </div>
    </div>
  );
