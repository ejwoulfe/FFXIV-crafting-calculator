...
export default function SearchList(recipes: SearchListProps) {
  ...
  function createListOfRecipes(list: Array<RecipeObject>) {
    if (list.length === 0) {
      return <p id="no-search-results"> No Search Results</p>;
    } else {
      ...
      return list.map((recipe: RecipeObject, index: number) => {
        ...
        return (
          <li>
            <Link
              to={`/disciple/${recipe.disciple_id}/recipes/recipe/${recipe.recipe_id}`}
  ...
  return (
    <ul id="recipe-list" data-testid="results-list">
